...
def main(board, words): 
    ...
    def search(word, i, j): 
        queue = deque()
        queue.append((i,j, 0, set()))
        rows = len(board)
        cols = len(board[0])
        moves = [(0,1), (0, -1), (1, 0), (-1, 0)]
        while len(queue) != 0: 
            x, y, l, visited = queue.popleft()
            if (x < 0 or x >= rows or 
                y < 0 or y >= cols or 
                (x,y) in visited or board[x][y] != word[l]) : 
                continue 
            if board[x][y] == word[l] and l == len(word) - 1: 
                return True 
            visited.add((x,y))
            for a, b in moves: 
                queue.append((a + x, y + b, l + 1, visited.copy()))
        return False 
    res = [] 
    for word in words: 
        if bfs(word): 
...
def solver(board, words): 
    ...
    def bfs(i, j): 
        queue = deque()
        queue.append((i,j, board[i][j], set()))
        rows = len(board)
        cols = len(board[0])
        moves = [(0,1), (0, -1), (1, 0), (-1, 0)]
        while len(queue) != 0: 
            x, y, cur, visited = queue.popleft()
            if ((x,y) in visited or len(cur) > 10): 
                continue 
            if cur in words: 
                res.append(cur)
                words.remove(cur)
            ...
            for a, b in moves: 
                x_move = x + a 
                y_move = y +b 
                if (x_move < 0 or x_move >= rows or y_move < 0 or y_move >= cols): 
                    continue 
                queue.append((a + x, y + b, cur + board[x_move][y_move], visited.copy()))
