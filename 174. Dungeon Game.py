...
def solver(dungeon): 
    ...
    res = 0 
    moves = [(0, 1), (1, 0)]
    queue = [(dungeon[0][0], (0,0))] # health, (pos)
    visited = {}
    while len(queue) != 0: 
        health, pos = heapq.heappop(queue)
        if pos in visited and visited[pos] <= health: 
            continue 
        visited[pos] = health
        res = max(health, res)
        ...
        for x, y in moves: 
            x += pos[0] 
            y += pos[-1] 
            if x < len(dungeon) and y < len(dungeon[0]): 
                heapq.heappush(queue, (dungeon[x][y] + health, (x,y))) 
    return res + 1
